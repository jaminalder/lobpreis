...
var files = fs.readdirSync('./chords/md');
for (var i = 0; i < files.length; i++) {

  const input_md = fs.readFileSync('./chords/md/' + files[i], { encoding: 'utf8' });
...
`
  ...
  console.log('./chords/html/' + path.parse(files[i]).name + '.html');
  fs.writeFileSync('./chords/html/' + path.parse(files[i]).name + '.html', output);
